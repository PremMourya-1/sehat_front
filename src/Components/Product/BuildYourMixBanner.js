...
import { FiCheckSquare, FiPackage, FiShoppingCart, FiSliders } from "react-icons/fi";
...
const STEPS = [
  ...
  { icon: FiCheckSquare, label: "Pick ingredients", detail: "Mix & match freely" },
  { icon: FiPackage, label: "Choose pack size", detail: "100g to bulk" },
  { icon: FiShoppingCart, label: "Add to cart", detail: "Fresh-packed & shipped" },
];
...
export default function BuildYourMixBanner() {
  return (
    <section className="relative overflow-hidden py-16 text-(--surface) max-md:py-12">
      <Image src={buildYourMixImage} alt="" fill sizes="100vw" className="object-cover" />
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col items-center gap-10 px-4 md:flex-row md:justify-between md:px-8">
        <div className="max-w-lg text-center md:text-left">
          <p className="font-accent text-lg text-(--accent) md:text-xl">Make it yours</p>
          <h2 className="mt-2 font-heading text-3xl max-md:text-2xl md:text-4xl">Build Your Own Mix</h2>
          <p className="mt-3 text-(--surface)/85">
            Choose a base of nuts or seeds, pick your favorite ingredients, select a pack
            size, and we'll pack it fresh — just for you.
          </p>
